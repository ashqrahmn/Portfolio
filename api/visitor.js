export default async function handler(req, res) {
  ...
  try {
    ...
    if (!visitor_id) {
      ...
      const isLocalIp = ip === "::1" || ip === "127.0.0.1" || ip === "";

      let isp = "", country = "", region = "", city = "", vpn_suspicious = false;

      if (!isLocalIp) {
        try {
          const geoRes = await fetch(
            `http://ip-api.com/json/${ip}?fields=status,message,country,regionName,city,isp,proxy`
          );
          ...
          if (geoData.status === "success") {
            ...
            city = geoData.city || "";
            vpn_suspicious = geoData.proxy === true;
          } else {
            console.warn("ip-api lookup failed:", geoData.message, "for IP:", ip);
          }
        } catch (geoErr) {
